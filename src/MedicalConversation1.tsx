...
// import { Mic, MicOff, Brain, RotateCcw, Stethoscope, FileText } from "lucide-react";
...
//     setLiveText("");
//     setSummary("");
//     setKeywords([]); // Also reset keywords when cleaning up
//   };
...
//     fetchKeywords(conversationText, setKeywords, setSummary, setMedicalRecord);
//     cleanupAll();
//   };
...
//             {/* Keywords Section - Moved here, after controls */}
//             {keywords.length > 0 && (
//               <div className="mt-6">
...
//             {/* Recorded Clips - Moved to last */}
//             <div className="mt-6">
//               <h3 className="font-semibold text-gray-800 mb-3">Recorded Audio Chunks:</h3>
...
//                       <audio controls src={URL.createObjectURL(blob)} className="flex-1" />
//                     </div>
//                   ))
//                 )}
//               </div>
...
const VoiceRecognizeScreen: React.FC = () => {
  ...
  useEffect(() => {
    return () => {
      ...
      if (speechRef.current && isSpeechRunningRef.current) {
        speechRef.current.stop();
      }
    };
  }, []);
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
            ...
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-200">
              <div className="flex space-x-3">
                <button
                  ...
                  className={`flex items-center space-x-2 px-6 py-3 rounded-lg ${status === "Recording…" ? 'bg-gray-300 text-slate-700 cursor-not-allowed' : 'bg-red-500 text-white hover:bg-red-600 cursor-pointer'}`}
                >
                  {status === "Recording…" ? <MicOff size={20} /> : <Mic size={20} />}
                  <span>{status === "Recording…" ? "Recording..." : "Start Recording"}</span>
                </button>

                <button
                  onClick={stopRecording}
                  disabled={status !== "Recording…"}
                  className={`flex items-center space-x-2 px-6 py-3 rounded-lg ${status !== "Recording…" ? 'bg-gray-300 text-slate-700 cursor-not-allowed' : 'bg-blue-500 text-white hover:bg-blue-600 cursor-pointer'}`}
                >
                  <Brain size={20} />
                  <span>Stop & Process</span>
                </button>
            ...
            <div className="mt-4 flex items-center space-x-2">
              <div className={`w-3 h-3 rounded-full ${status === "Recording…" ? 'bg-red-500 animate-pulse' : 'bg-gray-400'}`}></div>
